/*
 * Modified to account for wind or water current
 *
 *
...
public: void Load(const EntityComponentManager &_ecm,
            const sdf::ElementPtr &_sdf)
  {
    ...
    this->rho = _sdf->Get<double>("density", this->rho).first;
    ...
    {
      this->flow.subcribeToCurrent(_sdf->GetElement("flow_topic")->Get<std::string>());
    }
    else
    {
      ...
      gzwarn << "boating::LiftDrag plugin attached to [" << linkName << "] has no 'flow_topic' parameter specified. "
             << "No wind or water current will be considered.\n";
    }
...
{
private:
  std::unique_ptr<LiftDragPrivate> dataPtr{std::make_unique<LiftDragPrivate>()};
...
GZ_ADD_PLUGIN(boating::LiftDrag,
              System,
              ISystemConfigure,
              ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(boating::LiftDrag, "boating::LiftDrag")
